...
@section('title', 'Produk Edit Kategori')
...
<div class="card max-w-md">
  <form action="{{ route('productCategory.update', ['productCategory' => $productCategory]) }}" method="POST">
    @csrf
    @method('PUT')
    <div class="mb-3">
      <label for="name" class="form-label">Nama Kategori Produk</label>
      <input required type="text" id="name" name="name" value="{{ $productCategory->name }}" class="form-control @error('name') form-error @enderror">
      @error('name')
