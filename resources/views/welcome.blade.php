@extends('layouts.app')

@section('content')
<div class="text-center mt-5 pt-5">
    <h1>E-MKM</h1>
    <p>Karena Semua Pengusaha Hebat Pernah Jadi Pemula.</p>
</div>

<!-- CTA SECTION -->
<section
    class="cta-section"
    style="text-align: center; padding: 60px 20px; background-color: #f3f4f6"
>
    <h2 style="font-size: 2.2em; margin-bottom: 20px; color: #333">
        Siap Melesatkan Bisnismu?
    </h2>
    <p style="font-size: 1.2em; margin-bottom: 30px; color: #555">
        Gabung bersama E-MKM dan mulai perjalanan usahamu ke level berikutnya.
        Mudah & cepat untuk kamu, Sang Pengusaha.
    </p>
    <a class="btn btn-info"
        href="#daftar"
        style="
            color: white;
            padding: 15px 30px;
            font-size: 1em;
            border-radius: 5px;
            text-decoration: none;
        "
    >
        Daftar Sekarang
    </a>
</section>

<!-- FOOTER / CREDIT -->
<h5 class="text-center mt-4 mb-5">
    SUPPORTED BY HIPMI PT IDELPKIA
    <svg
        xmlns="http://www.w3.org/2000/svg"
        width="16"
        height="16"
        fill="currentColor"
        class="bi bi-box2-heart-fill"
        viewBox="0 0 16 16"
    >
        <path
            d="M3.75 0a1 1 0 0 0-.8.4L.1 4.2a.5.5 0 0 0-.1.3V15a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1V4.5a.5.5 0 0 0-.1-.3L13.05.4a1 1 0 0 0-.8-.4zM8.5 4h6l.5.667V5H1v-.333L1.5 4h6V1h1zM8 7.993c1.664-1.711 5.825 1.283 0 5.132-5.825-3.85-1.664-6.843 0-5.132"
        />
    </svg>
</h5>
@endsection
